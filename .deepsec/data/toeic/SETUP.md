# Agent setup for `toeic`

This is a deepsec scanning workspace. Project `toeic` was just registered
(target: `..`). Setup is incomplete — `data/toeic/INFO.md`
...
2. **Fill in `data/toeic/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id toeic
pnpm deepsec process --project-id toeic
```
