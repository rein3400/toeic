...
if (!$is_logged_in && $allow_public_certificates) {
    ...
    if ($public_session) {
        $stmt = $conn->prepare('SELECT id FROM toeic_test_results WHERE test_session = ?');
...
$cert_settings['cert_template']        = normalizeCertificateTemplate($cert_settings['cert_template']);
...
if ($previewMode) {
    $result = [
        'test_session'     => 'PREVIEW_RL_SESSION',
        'user_id'          => 0,
        'listening_scaled' => 415,
        'reading_scaled'   => 380,
        'total_score'      => 795,
        'cefr_level'       => 'B2',
    ...
    if ($public_access || ($_SESSION['role'] ?? '') === 'admin') {
        $stmt = $conn->prepare("
            SELECT r.*, u.full_name, u.username, s.status, s.practice_mode
...
if (!$previewMode) {
    $access = getCertificateAccessState($result, $result['status'] ?? null, 'rl');
    if (!$access['allowed']) {
...
function toeicRlScoreLevel(int $total): array {
    if ($total >= 945) return ['Advanced', 'A1'];
    if ($total >= 785) return ['Advanced', 'A2'];
    if ($total >= 605) return ['Working', 'B1'];
    if ($total >= 405) return ['Developing', 'B2'];
    if ($total >= 255) return ['Foundation', 'C1'];
    return ['Beginner', 'C2'];
}

$levelData = toeicRlScoreLevel($totalScore);
...
// Build a public verification URL and QR code for the certificate.
...
$pdfData = [
    ...
    'section_a_score'  => (int)($result['listening_scaled'] ?? 0),
    'section_b_label'  => 'Reading',
    'section_b_score'  => (int)($result['reading_scaled'] ?? 0),
    'cert_date'        => $cert_date,
...
if ($forceHtml) {
    ...
    <head>
        <meta charset="UTF-8">
        <title><?php echo htmlspecialchars($website_title); ?> Certificate - <?php echo htmlspecialchars($pdfData['full_name']); ?></title>
    ...
    <body>
        <div class="actions-bar">
            <div class="left"><i class="fas fa-certificate me-1"></i> TOEIC L&amp;R Certificate Preview</div>
            <div class="right">
        ...
        <div class="actions">
            ...
            <a href="result_toeic.php?session=<?php echo urlencode($result['test_session']); ?>" class="btn btn-back"><i class="fas fa-arrow-left"></i> Back to Results</a>
        </div>
    </body>
    </html>
    <?php
...
$pdfHtml = renderCertificatePdfHtml($pdfData);
$filename = 'TOEIC_Certificate_' . preg_replace('/[^a-zA-Z0-9]/', '_', $pdfData['full_name']) . '_' . preg_replace('/[^a-zA-Z0-9]/', '_', $result['test_session']) . '.pdf';
